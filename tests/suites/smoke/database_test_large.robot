...
Verify Query - Get results as a list of dictionarie v1
    [Tags]    db    smoke
    # ${output} =    Query    SELECT * FROM person;    \    True # works too
    ${output} =    Query    SELECT * FROM person;    returnAsDict=True
    Log    ${output}
    Should Be Equal As Strings    ${output[0]['first_name']}    Franz Allan
    Should Be Equal As Strings    ${output[1]['last_name']}    Schneider
...
Verify Query - Get results as a list of dictionaries v2
    ${output}=    Query    SELECT * FROM person    returnAsDict=True
    Log    ${output}
